...
const MainWindow = () => {
  const { content, activeFolder } = useContext(ExplorerContext);
  console.log(content);

  return (
    <div className="w-full h-full overflow-y-auto p-4 grid grid-cols-2 md:grid-cols-6 lg:grid-cols-4 xl:grid-cols-6 scroll-thin gap-2 md:gap-4">
      {content?.map((data) => (
        <button
          key={data?.id}
